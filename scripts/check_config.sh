...
case "$TARGET_ENV" in
  dev)
    ...
    check_no_placeholder "$WRANGLER_TOML" 'dev\.mail\.example\.com' "wrangler.toml dev SES_FROM_DOMAIN"
    ;;
  staging)
    check_no_placeholder "$WRANGLER_TOML" "REPLACE_WITH_STAGING_D1_DATABASE_ID" "wrangler.toml staging environment"
    check_no_placeholder "$WRANGLER_TOML" 'staging\.mail\.example\.com' "wrangler.toml staging SES_FROM_DOMAIN"
  ...
  production|prod)
    ...
    check_no_placeholder "$WRANGLER_TOML" 'mail\.example\.com' "wrangler.toml production SES_FROM_DOMAIN"
