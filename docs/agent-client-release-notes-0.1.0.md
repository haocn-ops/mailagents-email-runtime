...
Mailagents runtime capabilities, using the mailbox-first HTTP facade, and
calling MCP tools with a small typed helper surface.

The first release intentionally stays narrow. It focuses on runtime discovery,
compatibility contract access, mailbox-first read/send/reply flows, high-value
MCP calls, and stable error-code branching.
...
- high-level HTTP send and reply helpers
- MCP `tools/list`
...
- external agent builders integrating with Mailagents mailbox-first HTTP and MCP surfaces
